...
actual class DatabaseHelper private constructor(val context: Context) {
    actual val liveDatabase
        get() = _db!!
    ...
    actual suspend fun loadExternalDatabase(dbFilePath: Path) =
        Room.databaseBuilder(
            context,
            ChineseWordsDatabase::class.java,
            "Temp_HSK_DB_${UUID.randomUUID()}"
        )
            .createFromFile(File(dbFilePath.toString())) // instead of fromAsset
            .build()

    suspend fun loadExternalDatabase(stream: Callable<InputStream>) =
        Room.databaseBuilder(
            context,
            ChineseWordsDatabase::class.java,
            "Temp_HSK_DB_${UUID.randomUUID()}"
    ...
    actual suspend fun updateDatabaseFileOnDisk(newDatabasePath: Path)
        = withContext(Dispatchers.IO) {
            val newFile = newDatabasePath.toFile()
    ...
    actual suspend fun replaceUserDataInDB(dbToUpdate: ChineseWordsDatabase, updateWith: ChineseWordsDatabase) {
        withContext(Dispatchers.IO) {
    ...
    actual suspend fun replaceWordsDataInDB(dbToUpdate: ChineseWordsDatabase, updateWith: ChineseWordsDatabase)
        = withContext(Dispatchers.IO) {
    ...
    actual suspend fun snapshotDatabase(): Path {
        var snapshotFile : Path? = null

        withContext(Dispatchers.IO) {
            liveDatabase.flushToDisk()

            snapshotFile = FileUtils.copyUriToCacheDir(
                context,
                File(DATABASE_LIVE_PATH).toUri()
            ).toString().toPath()
        }
